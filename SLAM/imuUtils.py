...
#Function that merge all the data from multiple csv files into lists and numpy arrays 
# NOTE PLS clean the code if you have time so that it return the same type i.e. arrays or lists  
def merge_data(path):
    ...
    for file in tqdm(sorted(os.listdir(path), key=lambda x: int(x[:-4]))):
        ...
        #Reading the needed data from the pandas dataframe
...
#Calculates the rotation matrix from world cordinate to IMU frame to rotate the gravitational vector to IMU frame to subtract the gravity component for the relevant axis. 
